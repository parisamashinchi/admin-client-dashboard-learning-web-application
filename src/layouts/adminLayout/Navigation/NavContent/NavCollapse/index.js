...
class NavCollapse extends Component {
  ...
  render() {
    ...
    if (this.props.collapse.icon) {
      itemTitle = (
        <span className="pcoded-mtext">
          <IntlMessages id={this.props.collapse.title} />
        </span>
      );
    }
    ...
    const currentIndex = document.location.pathname
      ...
      .findIndex(id => id === this.props.collapse.id);
    if (currentIndex > -1) {
      navItemClass = [...navItemClass, "active", "pcoded-trigger"];
      if (this.props.layout !== "horizontal") {
        navLinkClass = [...navLinkClass, "active"];
      }
    }
...
const mapStateToProps = state => {
  return {
    layout: state.getIn(["PANEL", "layout"]),
    isOpen: state.getIn(["PANEL", "isOpen"]),
    isTrigger: state.getIn(["PANEL", "isTrigger"]),
  };
};
...
const mapDispatchToProps = dispatch => {
  return {
    onCollapseToggle: (id, type) =>
      dispatch({
        type: panelActions.COLLAPSE_TOGGLE,
        menu: { id: id, type: type },
      }),
    onNavCollapseLeave: (id, type) =>
      dispatch({
        type: panelActions.NAV_COLLAPSE_LEAVE,
        menu: { id: id, type: type },
      }),
  };
};
